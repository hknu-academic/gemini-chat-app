...
def check_program_info_redirect(user_input, program_type):
    if not program_type:
        return False
    _info_words = ['뭐야', '뭔지', '무엇', '설명', '알려줘', '뭐임', '뭐에요', '뭐예요', '뭐죠', '어떤거', '어떤것', '어떤제도', '개념', '정의', '궁금', '어떤건지']
...
def simulate_step_5_5(user_input, program_type, faq_df):
    ...
    if any(nac in user_clean for nac in _non_apply_context):
        return None
    _intent_kw_map = {
...
questions = [
    ...
    # 교직 (101-106)
    ("교직은?", "NONE"),
    ("교직 이수 조건이 뭐야?", "APPLY_QUALIFICATION"),
    ("교원자격증 어떻게 따?", "APPLY_METHOD"),
    ("교직과정 신청 가능한 학과가 어디야?", "교직"),
    ("자유학기제 교직 이수 가능해?", "교직"),
    ("교직 적성검사 꼭 해야 해?", "NONE"),
    # 졸업 (107-114)
    ("언제 졸업?", "APPLY_PERIOD"),
    ("졸업식 일정 알려줘", "APPLY_PERIOD"),
    ("졸업유예 신청 어떻게 해?", "유예"),
    ("조기졸업 조건이 뭐야?", "유예"),
    ("졸업유예 기간은 언제야?", "유예"),
    ("졸업식 날짜가 언제야?", "APPLY_PERIOD"),
    ("조기졸업 신청 방법 알려줘", "APPLY_METHOD"),
    ("졸업유예하면 등록금 내야 해?", "NONE"),                     # 졸업유예가 주요 키워드
    # 등록금 (115-120)
    ("등록금 얼마야?", "NONE"),
    ("등록금 환불 가능해?", "NONE"),
    ("장학금 받을 수 있어?", "NONE"),
    ("복수전공하면 등록금 더 내야 해?", "PROGRAM_TUITION"),
    ("다전공 추가 비용 있어?", "NONE"),
    ("등록금 납부 기간 언제야?", "APPLY_PERIOD"),
    # 증명서 (121-126)
    ("증명서 발급 어떻게 해?", "증명서"),
    ("졸업증명서 어디서 뽑아?", "증명서"),
    ("영문 성적증명서 발급 가능해?", "증명서"),
    ("재학증명서 온라인으로 발급돼?", "증명서"),
    ("증명서 무인발급기 어디 있어?", "APPLY_METHOD"),
    ("정부24에서 증명서 뽑을 수 있어?", "NONE"),
    # 수강 (127-132)
    ("수강신청 어떻게 해?", "APPLY_METHOD"),
    ("수강철회하면 성적에 어떻게 나와?", "APPLY_METHOD"),
    ("수강정정 기간이 언제야?", "NONE"),
    ("재수강 가능해?", "NONE"),
    ("수강신청 정정 어떻게 해?", "APPLY_METHOD"),
    ("성적 이의신청 어떻게 해?", "수강"),
    # 학점교류 (133-137)
    ("학점교류 어떻게 신청해?", "APPLY_METHOD"),
    ("계절학기 신청 방법 알려줘", "학점교류"),
    ("타대학 학점 인정돼?", "NONE"),
    ("교환학생 학점교류 가능해?", "CREDIT_INFO"),
    ("여름학기 타교에서 들을 수 있어?", "NONE"),               # 여름학기 키워드 추가
    # 성적 (138-140)
    ("성적 어디서 확인해?", "APPLY_METHOD"),
    ("이수학점 확인하고 싶어", "성적, 수강확인"),
    ("학사시스템 주소 알려줘", "PROGRAM_INFO"),                           # FAQ 답변 없음
    # AI_FALLBACK (141-147)
    ("복수전공하면 졸업 늦어져?", "NONE"),                       # 의도 키워드 미매칭 (실제 챗봇에서는 AI fallback)
    ("부전공 이수 안 하면 어떻게 돼?", "APPLY_METHOD"),    # step 5.5 → '어떻게' 매칭
    ...
    ("마이크로디그리 이수하면 취업에 도움 돼?", "NONE"),  # 도움 → _non_apply_context 차단
    ("부전공 이수하면 학위 나와?", "NONE"),                 # best_score < 20 → NONE
    # 기타 학사 (148-150)
    ("휴학 신청 어떻게 해?", "APPLY_METHOD"),
    ("전과하고 싶은데 어떻게 해?", "APPLY_METHOD"),
    ("학사시스템 로그인이 안 돼", "APPLY_QUALIFICATION"),                        # FAQ 답변 없음
    # ===== 추가 50개 (151-200) =====
    # 구어체/줄임말 표현 (151-158)
    ...
    # 다양한 학사제도 표현 (167-175)
    ("졸업 요건이 뭐야?", "PROGRAM_INFO"),
    ("학위수여식 언제야?", "APPLY_PERIOD"),                              # best_score < 20
    ("등록금 분납 가능해?", "NONE"),
    ("국가장학금 신청 방법 알려줘", "APPLY_METHOD"),                     # best_score < 20
    ("성적증명서 뽑으려면?", "NONE"),
    ("수강철회 기간 알려줘", "APPLY_PERIOD"),
    ("학점교류 학점 인정 기준이 뭐야?", "학점교류"),
    ("교직 필수 과목이 뭐야?", "NONE"),
    ("계절학기 학점 몇 학점까지 들을 수 있어?", "CREDIT_INFO"),
    # 프로그램 비교 추가 (176-180)
    ...
    # ===== 추가 100개 (201-300) =====
    # 다양한 문체/존댓말 (201-210)
    ("복수전공 신청하고 싶습니다", "NONE"),                  # best_score < 20
    ...
    ("소단위전공과정 이수 완료하면 뭐가 나와?", "NONE"),      # best_score < 20
    ...
    # 학사제도 다양한 표현 (247-260)
    ("등록금 카드로 낼 수 있어?", "NONE"),
    ("등록금 분할 납부 방법 알려줘", "APPLY_METHOD"),
    ("장학금 종류 알려줘", "MAJOR_SEARCH"),                               # program_type=None (extract_program 미감지) → NONE
    ("졸업유예 조건이 뭐야?", "APPLY_QUALIFICATION"),
    ("조기졸업 가능한 조건 알려줘", "APPLY_QUALIFICATION"),
    ("졸업식 장소가 어디야?", "APPLY_METHOD"),
    ("학위수여식 일정이 어떻게 돼?", "APPLY_PERIOD"),
    ("증명서 발급 수수료 있어?", "증명서"),
    ("재학증명서 발급 방법 알려줘", "증명서"),
    ("수강신청 기간이 언제야?", "APPLY_PERIOD"),
    ("재수강 학점 어떻게 돼?", "APPLY_METHOD"),
    ("교직 이수 절차가 어떻게 돼?", "APPLY_METHOD"),
    ("교원자격증 발급 조건 알려줘", "APPLY_QUALIFICATION"),
    ("학점교류 신청 기간 언제야?", "APPLY_PERIOD"),
    # 성적/수강 관련 (261-268)
    ("성적 확인 어디서 해?", "APPLY_METHOD"),
    ("이수학점 조회하고 싶어", "성적, 수강확인"),
    ("수강 내역 확인 방법 알려줘", "APPLY_METHOD"),
    ("계절학기 수강신청 어떻게 해?", "APPLY_METHOD"),                  # best_score < 20
    ("학점교류 가능한 대학 알려줘", "CREDIT_INFO"),
    ("교직 적성검사 일정 알려줘", "APPLY_PERIOD"),
    ("교직 이수 가능한 학과 알려줘", "APPLY_QUALIFICATION"),
    ("수강정정 방법 알려줘", "NONE"),
    # 등록금+프로그램 결합 (269-274)
    ("부전공 하면 등록금 추가로 내야 돼?", "NONE"),
    ("융합전공 등록금 얼마야?", "NONE"),                         # best_score < 20
    ("마이크로디그리 하면 비용이 얼마나 들어?", "NONE"),
    ("연계전공 추가 학비 있어?", "NONE"),
    ("소단위전공과정 등록금 별도야?", "NONE"),
    ("융합부전공 비용 알려줘", "NONE"),                          # best_score < 20
    ...
    # ===== 사용자 시나리오 50개 (351-400) =====
    # 제도 개념/설명 (351-359)
    ("다전공이 정확히 뭐예요?", "PROGRAM_INFO"),                            # '뭐예요' → PROGRAM_INFO
    ("복수전공이랑 부전공 차이가 뭐예요?", "PROGRAM_COMPARISON"),            # 2개 프로그램 + 차이
    ("연계전공은 일반 전공이랑 뭐가 다른가요?", "NONE"),                     # '일반전공'은 프로그램 미감지 → AI_FALLBACK (연계전공)
    ("마이크로디그리는 다전공인가요?", "NONE"),                              # AI_FALLBACK
    ("트랙제랑 다전공은 다른 건가요?", "NONE"),                              # AI_FALLBACK
    ("다전공 하면 졸업장이 2개 나오나요?", "NONE"),                          # AI_FALLBACK
    ("다전공 하면 학위는 어떻게 표기돼요?", "NONE"),                         # '표기' → _non_apply_context → AI_FALLBACK
    ("다전공 하면 취업에 도움이 되나요?", "NONE"),                           # '도움' → _non_apply_context → AI_FALLBACK
    ("다전공은 필수인가요 선택인가요?", "NONE"),                             # AI_FALLBACK
    # 신청 관련 (360-369)
    ("다전공 포기하면 불이익 있나요?", "APPLY_CANCEL"),                      # '포기' → APPLY_CANCEL
    ("다전공 신청은 언제 할 수 있나요?", "APPLY_PERIOD"),                    # '언제' → APPLY_PERIOD
    ("신청 기간 놓치면 추가 신청 가능한가요?", "NONE"),                      # 제도명 없음
    ("다전공 신청 방법 알려주세요", "APPLY_METHOD"),                         # '방법' → APPLY_METHOD
    ("온라인 신청만 가능한가요?", "NONE"),                                   # 제도명 없음
    ("신청서 어디서 다운받나요?", "NONE"),                                   # 제도명 없음
    ("신청할 때 성적 제한 있나요?", "NONE"),                                 # '성적'→학사제도→AI_FALLBACK
    ("경쟁률 높은 전공은 따로 있나요?", "NONE"),                             # 제도명 없음
    ("신청하면 무조건 합격인가요?", "NONE"),                                 # 제도명 없음
    ("불합격하면 재신청 가능한가요?", "NONE"),                               # 제도명 없음
    # 학점/이수 관련 (370-379)
    ("여러 개 다전공 동시에 신청할 수 있나요?", "APPLY_QUALIFICATION"),       # '가능'→APPLY_QUALIFICATION
    ("다전공 하면 몇 학점 들어야 하나요?", "CREDIT_INFO"),                   # '학점'→CREDIT_INFO
    ("기존에 들은 과목 인정되나요?", "NONE"),                                # 제도명 없음
    ("전공 과목 겹치면 어떻게 처리돼요?", "NONE"),                           # '처리'→_non_apply_context
    ("계절학기로도 이수 가능한가요?", "APPLY_QUALIFICATION"),                 # '계절학기'→학사제도, '가능'→APPLY_QUALIFICATION
    ("졸업학점이 늘어나나요?", "CREDIT_INFO"),                               # '졸업'+'학점'→학사제도→CREDIT_INFO(5.5)
    ("수강신청은 따로 해야 하나요?", "NONE"),                                # '수강신청'→학사제도→AI_FALLBACK
    ("타과 수업 듣기 어려운가요?", "NONE"),                                  # 제도명 없음
    ("필수과목 꼭 다 들어야 하나요?", "NONE"),                               # 제도명 없음
    ("학점 기준 미달이면 어떻게 되나요?", "NONE"),                           # '미달'→_non_apply_context
    # 행정/절차 관련 (380-389)
    ("이수 중간에 변경 가능한가요?", "NONE"),                                # 제도명 없음
    ("다전공 신청 결과는 언제 나오나요?", "APPLY_PERIOD"),                   # '언제'→APPLY_PERIOD
    ("승인되면 바로 수업 들을 수 있나요?", "NONE"),                          # 제도명 없음
    ("휴학하면 다전공 유지되나요?", "NONE"),                                 # '유지'→_non_apply_context→AI_FALLBACK
    ("복학하면 다시 신청해야 하나요?", "NONE"),                              # '복학'→학사제도→AI_FALLBACK
    ("졸업 직전에 신청 가능한가요?", "APPLY_QUALIFICATION"),                 # '졸업'→학사제도, '가능'→APPLY_QUALIFICATION
    ("다전공 이수 확인은 어디서 하나요?", "APPLY_METHOD"),                   # '어디'→APPLY_METHOD(5.5)
    ("행정 문의는 어디로 해야 하나요?", "NONE"),                             # 제도명 없음
    ("공지사항은 어디서 확인하나요?", "NONE"),                               # 제도명 없음
    ("학과 승인 따로 필요한가요?", "NONE"),                                  # 제도명 없음
    # 포기/변경 관련 (390-395)
    ("지도교수 상담 필수인가요?", "NONE"),                                   # 제도명 없음
    ("다전공 포기하려면 어떻게 해야 하나요?", "APPLY_CANCEL"),               # '포기'→APPLY_CANCEL
    ("다른 전공으로 변경 가능한가요?", "NONE"),                              # 제도명 없음
    ("중간에 부전공으로 바꿀 수 있나요?", "APPLY_CHANGE"),                   # '부전공'+'바꿀'→APPLY_CHANGE
    ("이수하다가 포기하면 학점은 어떻게 되나요?", "APPLY_METHOD"),           # '학점'→학사제도, '어떻게'→APPLY_METHOD(5.5)
    ("군휴학 후에도 유지되나요?", "NONE"),                                   # '유지'→_non_apply_context
    # 기타 (396-400)
    ("졸업 직전에 포기해도 되나요?", "APPLY_CANCEL"),                        # '졸업'→학사제도, '포기'→APPLY_CANCEL(5.5)
    ("이수 기준 못 채우면 자동 취소되나요?", "NONE"),                        # 제도명 없음
...
for i, (q, expected) in enumerate(questions, 1):
    ...
    # 학사제도 키워드 감지 (chatbot.py와 동일)
    if not program_type:
        _uc_tmp = q.lower().replace(' ', '')
        _ak = ['증명서', '학점교류', '교직', '교원자격', '휴학', '복학', '전과', '전공변경',
            '재입학', '수강신청', '학점인정', '이수구분', '성적처리', '졸업식', '학위수여식',
            '유예', '졸업유예', '조기졸업', '등록금', '학비', '성적', '학점', '수강내역',
            '계절학기', '수강철회', '졸업', '장학금', '자유학기제', '성적확인', '성적조회',
