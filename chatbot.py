...
def create_info_card(title, content_list, border_color="#007bff", emoji="📌"):
    """정보 카드 생성 - content_list는 문자열 리스트"""
    items_html = ""
...
def handle_course_search(user_input, extracted_info, data_dict):
    ...
    if not major:
        ...
        response = "## 📚 과목 조회\n\n"
        response += "어떤 전공의 과목을 찾으시나요?\n\n"
        response += "💡 **정확한 전공명을 입력해주세요.**\n\n"
        
        if available_majors:
            response += "📋 **조회 가능한 전공 예시:**\n"
            for m in available_majors:
                response += f"- {m}\n"
            response += "\n"
        
        response += "**예시 질문:** \"AI반도체융합전공 과목 알려줘\"\n\n"
    ...
    if major_courses.empty:
        ...
        response = f"## 📚 '{major}' 과목 조회 실패\n\n"
        response += f"입력하신 **'{major}'** 전공을 찾을 수 없습니다.\n\n"
        
        if similar_majors:
            response += "🔍 **혹시 이 전공을 찾으셨나요?**\n"
            for sm in similar_majors[:5]:
                response += f"- {sm}\n"
            response += "\n"
        else:
            # 전체 전공 목록 일부 표시
            all_majors = sorted(courses_data['전공명'].unique().tolist())[:8]
            response += "📋 **조회 가능한 전공:**\n"
            for m in all_majors:
                response += f"- {m}\n"
            response += "\n"
        
        response += "💡 **정확한 전공명을 다시 입력해주세요.**\n\n"
        response += CONTACT_MESSAGE
        return response, "COURSE_SEARCH"
...
def main():
    ...
    if menu == "AI챗봇 상담":
        ...
        with st.expander("💡 어떤 질문을 해야 할지 모르겠나요? (클릭)", expanded=False):
            ...
            <style>
            /* 신청 버튼 - 파란색 */
            div[data-testid="column"]:has(button[key^="qa_"]) button,
            button[kind="secondary"]:has-text("자격"),
            .stButton button {
        ...
        if selected_program:
            ...
            if available_majors:
                ...
                if selected_program in target_programs:
                    ...
                    with col_m1:
                        selected_major = st.selectbox(f"이수하려는 {selected_program}", sorted(available_majors.keys()))
                    with col_m2:
                        primary_list = []
